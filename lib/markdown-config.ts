...
// Default icons copied from remark-github-alerts (not exported by the package).
// Passing ANY `icons` object replaces the defaults entirely, so we must include all 5.
const GITHUB_ALERT_ICONS: Record<string, string> = {
  ...
  // PROMO title+icon hidden via CSS — just needs a valid SVG to prevent crash
  promo: '<svg class="octicon" viewBox="0 0 16 16" width="16" height="16"></svg>',
};
